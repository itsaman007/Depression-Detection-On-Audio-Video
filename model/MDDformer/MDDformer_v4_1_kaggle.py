"""
MDDformer-V4.1 - Kaggle Training Script (Self-Contained)
========================================================
Paper: "LMVD: A Large-Scale Multimodal Vlog Dataset for Depression Detection in the Wild"
       (arXiv: 2407.00024)

Phase 4.1: Improved Multi-Seed Ensemble with Weighted Averaging
V4 revealed that seed 3333 consistently underperforms by 5-8% per fold,
dragging down the ensemble. Simple logit averaging gives equal vote to a
70% model and an 81% model, which is suboptimal.

CHANGES FROM V4:
  1. Seeds: [2222, 3333, 4444] → [2222, 4444, 5555]
     - Seed 3333 removed: consistently 5-8% below seed 2222 in every fold
     - Seed 5555 added: fresh initialization for diversity
  2. Weighted Ensemble: weight_i = acc_i / sum(all_accs)
     - Each model's logits are weighted by its dev accuracy
     - Stronger models have more influence on predictions
     - Replaces naive averaging that let weak models drag down results

V4 Analysis (through 5 folds):
  - Seed 2222: 76.50, 74.32, 77.60, 75.82, 81.32 → avg 77.11% (dominant)
  - Seed 3333: 73.77, 72.68, 72.68, 70.88, 77.47 → avg 73.50% (problem)
  - Seed 4444: 74.86, 75.96, 69.95, 72.53, 81.32 → avg 74.92% (variable)
  - Ensemble only won 1/5 folds; individual seed 2222 won 4/5

Strategy:
  - Train 3 models per fold (seeds: 2222, 4444, 5555)
  - Each model uses the EXACT V2 architecture and hyperparameters
  - At test time: WEIGHTED average of logits (by dev accuracy), then argmax
  - TTA applied to each model independently before weighted averaging

Why weighted ensemble works better:
  - A model with 81% dev acc gets ~35% of vote vs ~30% for a 70% model
  - Prevents weak models from overriding correct predictions of strong models
  - Preserves diversity benefit while respecting model quality

What's removed vs V2:
  - SWA: never won any fold (removed in V3 too)
...
ENSEMBLE_SEEDS = [2222, 4444, 5555]   # 3 models per fold (dropped 3333, added 5555)
NUM_MODELS = len(ENSEMBLE_SEEDS)
USE_WEIGHTED_ENSEMBLE = True           # [V4.1] Weight logits by dev accuracy
...
# ==================== MDDformer-V4.1 (same as V2 Net) =================

class NetV4(nn.Module):
    """
    MDDformer-V4.1: IDENTICAL architecture to V2 (~990K params).
    The improvement comes from ENSEMBLE + WEIGHTED AVERAGING, not architecture change.
    """
    def __init__(self):
        super().__init__()
...
def ensemble_predict(models, videoData, audioData, weights=None,
    ...
    """
    [V4.1] Weighted ensemble prediction: weighted average of logits across models.

    Args:
        models: list of trained models
        videoData, audioData: input tensors
        weights: list of float weights (one per model), should sum to 1.0
                 If None, uses equal weights (fallback to V4 behavior)
        use_tta: whether to apply TTA per model
        tta_passes: number of TTA passes
        tta_drop_ratio: TTA temporal masking ratio

    Returns:
        Weighted average logits tensor
    """
    all_logits = []
...
def evaluate_ensemble(models, devLoader, device, weights=None,
                      use_tta=False, tta_passes=5, tta_drop_ratio=0.25):
    """
    [V4.1] Evaluate weighted ensemble of models on dev set.
...
def train_single_model(VideoPath, AudioPath, X_train, X_test, labelPath,
    ...
    if best_model_state is not None:
        ...
        save_name = f"MDDformer_v4_1_seed{model_seed}_{mytop:.2f}.pth"
...
def train_fold(VideoPath, AudioPath, X_train, X_test, labelPath, numkfold,
    ...
    Steps:
      ...
      3. Compute weights from dev accuracies
      4. Evaluate: individual → weighted ensemble → weighted ensemble+TTA
      5. Report best of all strategies
    """
    ...
    models = []
    valid_accs = []  # Track accs for models that loaded successfully
    ...
    # ---- Step 3: Compute weights from dev accuracies [V4.1 NEW] ----
...
if __name__ == '__main__':
    print("=" * 60)
    print("MDDformer-V4.1 Training — Weighted Multi-Seed Ensemble")
    ...
    log_dir = os.path.join(LOG_DIR, f'MDDformer_v4_1_{tim}')
    save_dir = os.path.join(SAVE_DIR, f'MDDformer_v4_1_{tim}')
    ...
    print(f"  Seeds: {ENSEMBLE_SEEDS} (dropped 3333, added 5555)")
    print(f"  Weighted Ensemble: {USE_WEIGHTED_ENSEMBLE} (weight by dev accuracy)")
    ...
    if len(all_pre) > 0:
        ...
        plot_confusion_matrix(total_label_np, total_pre_np, [0, 1],
                              ...
                              title='MDDformer-V4.1 Weighted Ensemble 10-Fold CM')
    ...
    print("\n" + "=" * 60)
    print("MDDformer-V4.1 Weighted Multi-Seed Ensemble 10-Fold CV Results")
    print("=" * 60)
    ...
    print(f"  V4:        ~77.5% (simple ensemble, seed 3333 problem)")
    print(f"  V4.1:      {avg_acc:.2f}% (weighted ensemble, better seeds)")
    improvement = avg_acc - 77.29
    print(f"  Delta V2→V4.1: {improvement:+.2f}%")
    print("=" * 60)

    # Save results to text file
    with open(os.path.join(log_dir, "results.txt"), "w") as f:
        f.write(f"MDDformer-V4.1 Weighted Multi-Seed Ensemble 10-Fold CV Results\n")
        f.write(f"{'='*50}\n")
        f.write(f"Ensemble: {NUM_MODELS} models per fold\n")
        f.write(f"Seeds: {ENSEMBLE_SEEDS}\n")
        f.write(f"Weighted Ensemble: {USE_WEIGHTED_ENSEMBLE}\n")
        ...
        f.write(f"  Seeds: {ENSEMBLE_SEEDS} (dropped 3333, added 5555)\n")
        f.write(f"  Weighted Ensemble: {USE_WEIGHTED_ENSEMBLE}\n")
