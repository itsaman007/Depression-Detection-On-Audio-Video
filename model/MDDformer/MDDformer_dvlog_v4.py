"""
MDDformer-V4 — Adapted for D-Vlog Dataset (10-Fold CV)
========================================================
V4 = V2 architecture × 3 seeds per fold + logit averaging ensemble.

No architectural changes from V2. Instead: train 3 independently-
seeded models per fold, then average their logits at test time.

Strategy:
  - Train 3 models per fold (seeds: 2222, 3333, 4444)
  - Each model uses exact V2 architecture + hyperparameters
  - At test time: average logits, then argmax
  - TTA applied per model independently before averaging

D-Vlog adaptations: same as baseline/V2 D-Vlog.

LMVD V4 result: 78.50% avg 10-fold accuracy
...
class NetV4(nn.Module):
    """MDDformer-V4 for D-Vlog. Architecture identical to V2."""
    def __init__(self):
        super().__init__()
...
def ensemble_predict(models, videoData, audioData, use_tta=False,
    ...
    """Average logits across multiple models. If TTA, each model does TTA independently."""
    all_logits = []
    for model in models:
        model.eval()
        if use_tta:
...
def train_single_model(dvlog_path, X_train, X_test, labels_dict,
    ...
    if best_model_state is not None:
        ...
        save_name = f"MDDformer_dvlog_v4_seed{model_seed}_{mytop:.2f}.pth"
...
def train_fold(dvlog_path, all_indices, all_labels, train_idx, test_idx,
    ...
    # Re-evaluate best individual with full metrics
...
if __name__ == '__main__':
    print("=" * 60)
    print("MDDformer-V4 — D-Vlog Dataset (10-Fold CV)")
    ...
    log_dir = os.path.join(LOG_DIR, f'MDDformer_dvlog_v4_10fold_{tim}')
    save_dir = os.path.join(SAVE_DIR, f'MDDformer_dvlog_v4_10fold_{tim}')
    ...
    if len(all_pre) > 0:
        ...
        plot_confusion_matrix(total_label_np, total_pre_np, [0, 1],
                              ...
                              title='MDDformer-V4 D-Vlog 10-Fold Confusion Matrix')
    ...
    print("\n" + "=" * 60)
    print("MDDformer-V4 — D-Vlog 10-Fold CV Results")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("COMPARISON: MDDformer-V4 on D-Vlog vs LMVD")
    print("=" * 60)
    print(f"{'Metric':<20} {'D-Vlog':>10} {'LMVD':>10}")
    print(f"{'-'*40}")
    print(f"{'Avg Accuracy':<20} {avg_acc:>9.2f}% {'78.50':>9}%")
    print(f"{'Avg F1':<20} {avg_f1:>10.4f} {'------':>10}")
    print("=" * 60)

    with open(os.path.join(log_dir, "results.txt"), "w") as f:
        f.write(f"MDDformer-V4 on D-Vlog — 10-Fold CV\n")
        f.write(f"{'='*50}\n")
