"""
MDDformer-V3.1 — Adapted for D-Vlog Dataset (10-Fold CV)
========================================================
V3.1 = V2 architecture + gentle R-Drop regularization.

Changes from V2:
  1. [REMOVED] SWA — never won any fold
  2. [CHANGED] TTA passes 5 → 10
  3. [NEW] R-Drop Regularization (alpha=1.0, warmup 20 epochs)

D-Vlog adaptations: same as baseline/V2 D-Vlog.

LMVD V3.1 result: 77.18% avg 10-fold accuracy
...
USE_ATTN_POOL = True
USE_TTA = True
TTA_PASSES = 10             # V3: increased from 5
TTA_DROP_RATIO = 0.25

# V3.1 R-Drop
USE_RDROP = True
RDROP_ALPHA = 1.0            # V3.1: reduced from 3.0
RDROP_WARMUP_EPOCHS = 20     # V3.1: no R-Drop for first N epochs
...
class NetV3(nn.Module):
    """MDDformer-V3.1 for D-Vlog. Architecture identical to V2."""
    def __init__(self):
        super().__init__()
...
def compute_rdrop_loss(logits1, logits2, labels, ce_loss_fn, alpha=RDROP_ALPHA):
    """R-Drop: CE + bidirectional KL divergence."""
...
def train_fold(dvlog_path, all_indices, all_labels, train_idx, test_idx,
    ...
    """Train MDDformer-V3.1 for one fold with R-Drop."""
    ...
    for epoch in range(1, epochSize + 1):
        ...
        for batch_idx, (visual, acoustic, label) in enumerate(trainLoader):
            ...
            if USE_RDROP:
                output1 = model(visual, acoustic)
                output2 = model(visual, acoustic)

                loss, ce_val, kl_val = compute_rdrop_loss(
        ...
        if epoch % testRows == 0:
            ...
            if epoch % 10 == 0 or epoch == 1:
                print(f"    Val Acc: {acc:.2f}% F1: {f1:.4f}")
            ...
            if acc > topacc:
                ...
                save_name = f"MDDformer_dvlog_v3_{epoch}_{float(acc):.2f}_{p:.4f}_{r:.4f}_{f1:.4f}.pth"
...
if __name__ == '__main__':
    print("=" * 60)
    print("MDDformer-V3.1 — D-Vlog Dataset (10-Fold CV)")
    print("R-Drop Regularization (alpha=1.0, warmup=20)")
    print("=" * 60)
    ...
    print(f"  R-Drop: {USE_RDROP} (alpha={RDROP_ALPHA}, warmup={RDROP_WARMUP_EPOCHS})")

    tim = time.strftime('%m_%d__%H_%M', time.localtime())
    log_dir = os.path.join(LOG_DIR, f'MDDformer_dvlog_v3_10fold_{tim}')
    save_dir = os.path.join(SAVE_DIR, f'MDDformer_dvlog_v3_10fold_{tim}')
    ...
    if len(all_pre) > 0:
        ...
        plot_confusion_matrix(total_label_np, total_pre_np, [0, 1],
                              ...
                              title='MDDformer-V3.1 D-Vlog 10-Fold Confusion Matrix')
    ...
    print("\n" + "=" * 60)
    print("MDDformer-V3.1 — D-Vlog 10-Fold CV Results")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("COMPARISON: MDDformer-V3.1 on D-Vlog vs LMVD")
    print("=" * 60)
    print(f"{'Metric':<20} {'D-Vlog':>10} {'LMVD':>10}")
    print(f"{'-'*40}")
    print(f"{'Avg Accuracy':<20} {avg_acc:>9.2f}% {'77.18':>9}%")
    print(f"{'Avg F1':<20} {avg_f1:>10.4f} {'------':>10}")
    print("=" * 60)

    with open(os.path.join(log_dir, "results.txt"), "w") as f:
        f.write(f"MDDformer-V3.1 on D-Vlog — 10-Fold CV\n")
        f.write(f"{'='*50}\n")
        ...
        f.write(f"\nConfig:\n")
        f.write(f"  R-Drop: alpha={RDROP_ALPHA}, warmup={RDROP_WARMUP_EPOCHS}\n")
        f.write(f"  TTA: passes={TTA_PASSES}\n")
        f.write(f"  Total params: {total_params:,}\n")
        f.write(f"\n--- COMPARISON ---\n")
        f.write(f"D-Vlog Avg Acc: {avg_acc:.2f}%\n")
        f.write(f"LMVD   Avg Acc: 77.18% (V3.1)\n")
